...
<section class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12" style="line-height:50px;">
   ...
   <a href="<?php echo $host?>phim-chieu-rap.html" class="text-warning font-weight-bold"> Phim Chiếu Rạp  </a> 
</section>
	<?php
		$limit = 8;
		if(isset($_GET['page']) && filter_var($_GET['page'], FILTER_VALIDATE_INT, array('min_range' => 1)))
		{
			...
			$sql = "SELECT * FROM phim WHERE MALOAI = 5 ORDER BY ID DESC LIMIT {$one_page}, {$limit}";
		}
		else
		{
			$page = 0;
			$sql = "SELECT * FROM phim WHERE MALOAI = 5 ORDER BY ID DESC LIMIT {$page}, {$limit}";
		}
    ...
    <div class="clearfix"></div><br />
   	...
   	<!-- Nút Trước -->
   		<li class="pre" style="<?php if(!isset($_GET['page']) or $_GET['page'] == 1) echo 'display: none;' ?>">
   			<a href="<?php echo $host;?>phim-chieu-rap.html?page=<?php if(isset($_GET['page']) && $_GET['page'] > 0) $previous = $_GET['page']; echo --$previous; ?>"><</a>
   		</li>
   	<?php
   		$sql3 = "SELECT ID, MALOAI FROM phim WHERE MALOAI = 5";
	...
	?>
			<li><a class="<?php if(!isset($_GET['page']) && $i == 1) echo 'active'; ?>" href="<?php echo $host;?>phim-chieu-rap.html?page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
	<?php
   		}
   	?>
   	<!-- Nút Next -->
   		<li class="nx" style="<?php if($_GET['page'] == $number_page or $number_page == 1) echo 'display: none;' ?>">
   			<a href="<?php echo $host;?>phim-chieu-rap.html?page=<?php if(!isset($_GET['page'])){echo 2;}elseif($_GET['page']){$next = $_GET['page']; echo ++$next;} ?>">></a>
   		</li>
 	</ul>
</section>
